...
export function ConsolidationReport() {
  ...
  const { data, isLoading } = useFulfillmentConsolidation(dateStr);
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      <html><head><title>Consolidation Report - ${format(selectedDate, "MMM dd, yyyy")}</title>
      <style>
      ...
      <p>Delivery Date: ${format(selectedDate, "MMMM dd, yyyy")} · ${consolidated.length} items · ${data?.orders?.length || 0} orders</p>
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `consolidation-${dateStr}.csv`;
    a.click();
      ...
      <CardHeader>
        ...
        {data?.orders && (
          <p className="text-sm text-muted-foreground">
            {data.orders.length} order(s) scheduled for delivery · {consolidated.length} unique item(s)
          </p>
        )}
      </CardHeader>
      <CardContent>
        {isLoading ? (
          ...
          <div className="py-12 text-center">
            ...
            <p className="mt-4 text-muted-foreground">No orders scheduled for {format(selectedDate, "MMM dd, yyyy")}</p>
          </div>
        ) : (
          <div ref={printRef}>
            <Table>
